/* Vercel Serverless Function — serves GET /api/news?city=London
   Fetches Google News RSS server-side and returns clean JSON.
   Deploy: import your repo on Vercel (free). Nothing else to set. */

export default async function handler(req, res) {
  const city = String((req.query && req.query.city) || "London").slice(0, 60);
  const feeds = [
  ...
  for (const f of feeds) {
    try {
      ...
      if (items.length) { res.status(200).send(JSON.stringify({ items })); return; }
    } catch (e) { /* try next feed */ }
  }
  res.status(502).send(JSON.stringify({ items: [] }));
}
